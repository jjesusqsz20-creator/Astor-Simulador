...
<div style="flex: 1; min-width: 250px; max-width: 400px; background-color: {CARD_BG}; border: 1px solid {GOLD_COLOR}; border-radius: 12px; padding: 25px; text-align: center; border-top: 5px solid {GOLD_COLOR}; box-shadow: 0 10px 25px rgba(0,0,0,0.4); height: 170px; display: flex; flex-direction: column; justify-content: center;">
...
<div style="flex: 1; min-width: 250px; max-width: 400px; background-color: {CARD_BG}; border: 1px solid {ACCENT_COLOR}; border-radius: 12px; padding: 25px; text-align: center; border-top: 5px solid {ACCENT_COLOR}; box-shadow: 0 10px 25px rgba(0,0,0,0.4); height: 170px; display: flex; flex-direction: column; justify-content: center;">
...
<div style="width: 100%; background: {CARD_BG}; border: 1px solid {GOLD_COLOR}33; border-left: 5px solid {GOLD_COLOR}; border-radius: 12px; padding: 30px; margin-bottom: 45px; box-shadow: 0 10px 40px rgba(0,0,0,0.5); text-align: center; position: relative; overflow: hidden;">
    <div style="position: absolute; top: 0; right: 0; width: 100px; height: 100px; background: radial-gradient(circle at top right, {GOLD_COLOR}15, transparent); pointer-events: none;"></div>
    <p style="color: {TEXT_COLOR}; font-size: 1.35rem; line-height: 1.6; margin: 0; font-family: 'Montserrat', sans-serif;">
        Para garantizar un retiro mensual de <span style="color: #34D399; font-weight: 800;">${renta_mensual_sidebar:,.0f}</span>, 
        es indispensable consolidar un fondo de inversión de <span style="color: {GOLD_COLOR}; font-weight: 800;">${meta_retiro:,.0f} ({meta_millones:.1f} millones)</span> 
        que genere un rendimiento de <span style="color: #34D399; font-weight: 800;">{rendimiento_anual}%</span> anuales.
    </p>
    <div style="height: 1px; background: linear-gradient(90deg, transparent, {GOLD_COLOR}44, transparent); margin: 25px auto; width: 70%;"></div>
    <p style="color: {TEXT_COLOR}; font-size: 1.35rem; line-height: 1.6; margin: 0; font-family: 'Montserrat', sans-serif; opacity: 0.95;">
        Por lo tanto, considerando que actualmente tienes <span style="color: {ACCENT_COLOR}; font-weight: 800;">{edad_inicial} años</span>, 
        el plan de acción requiere una aportación mensual de <span style="color: {ACCENT_COLOR}; font-weight: 800;">${aporte_m_metric:,.2f}</span>.
    </p>
</div>
...
if st.session_state.modulo_activo == "⚖️ Simulador Comparación":
    ...
    with st.sidebar:
        ...
        if st.button("⬅️ Volver al HUB", use_container_width=True):
            st.session_state.modulo_activo = "Hub"
            st.rerun()
...
""", unsafe_allow_html=True)
    ...
    if r_mensual_h > 0 and credito_estimado > 0:
        ...
        pago_hipoteca_mes = credito_estimado / n_meses_hipoteca
    else:
